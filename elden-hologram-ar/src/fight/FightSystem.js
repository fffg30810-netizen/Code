...
export class FightSystem {
  ...
  _pickMove(a, t) {
    ...
    let total = 0;
    for (const m of usable) total += m.weight || 1;
    let r = this.rng.next() * total;
    for (const m of usable) { r -= m.weight || 1; if (r <= 0) return m; }
  ...
  _startMove(a, t, move) {
    ...
    a.playMove(move.motion, { windup: move.windup, active: move.active, recovery: move.recovery });
  ...
  _threatened(a, alive) {
    for (const o of alive) {
      ...
      const left = f.move.windup - f.moveTime;
  ...
  _updateFighter(a, alive, dt) {
    ...
    switch (f.state) {
      case 'approach': {
        ...
        if (reach > target) {
          const speed = a.stats.speed * a.height * (0.9 + traits.aggression * 0.5);
      ...
      case 'active': {
        const m = f.move;
        const prev = f.moveTime - m.windup;
        f.moveTime += dt;
        const k = clamp((f.moveTime - m.windup) / Math.max(1e-4, m.active), 0, 1);
        ...
        if (f.moveTime >= m.windup + m.active) {
      ...
      case 'evade': {
        ...
        if (f.moveTime >= moveDuration(m)) { f.move = null; f.state = 'approach'; f.decision = this.rng.range(0.1, 0.4); a.play('idle'); }
        break;
      }
  ...
  _resolveHit(a, t, move, alive, multi) {
    ...
    for (const victim of targets) {
      ...
      if (victim.fight.state === 'evade' && victim.fight.moveTime < (victim.fight.move ? moveDuration(victim.fight.move) * 0.8 : 0.3)) {
      ...
      if (wasAlive && !victim.alive) { this.hooks.onDeath && this.hooks.onDeath(victim, a); continue; }

      if (!guarded && victim.alive) {
        victim.fight.poise -= (move.poise || 1) * (multi ? 0.35 : 1);
        const interruptible = victim.fight.state !== 'active' && victim.fight.state !== 'phase2';
        if (victim.fight.poise <= 0 && interruptible) {
          victim.fight.poise = victim.fight.maxPoise;
          victim.fight.state = 'stagger';
          victim.fight.staggerTimer = 0.75;
          victim.fight.move = null;
          victim.play('hit', { loop: false, fade: 0.05 });
  ...
  _launchProjectile(a, t, move) {
    ...
    this.pending.push({
      ...
      run: () => {
        ...
        else if (t.alive && t.fight.state !== 'active') t.play('hit', { loop: false, fade: 0.05 });
      },
    });
  }
