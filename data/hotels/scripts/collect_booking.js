...
function buildUrl(checkin, checkout, nflt) {
...
(async () => {
  ...
  for (const checkin of dates()) {
    ...
    // hostels: private rooms only appear when the cheapest matching unit is private; ask for private bathroom to surface private rooms
    queries.push(await fetchQuery(checkin, checkout, `distance=3000;ht_id=203;roomfacility=38`, 'q4_hostel_privbath')); await sleep(1500);
    const c = cand();
